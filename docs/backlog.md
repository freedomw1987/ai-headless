...
| **TD-502** | Tech Debt | AI API 缺乏 server-side 驗證 + rate limit | `/api/chat/stream` 未檢查 Auth、未限速、未審計。P1 安全風險 | P1 | 1 | M5 | Ready for Sprint 5 |
| **TD-503** | Tech Debt | SSE 串流無 abort/cancel 機制 | 用戶離開頁面或新對話時，串流繼續消耗 API quota | P2 | 1 | M5 | Ready for Sprint 5 |
