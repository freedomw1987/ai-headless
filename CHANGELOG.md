...
## Sprint 14 — Runtime 取代 Compiler（徹底撤手寫）

**Goal**：從 compiler 路線轉 runtime 路線，撤除 Sprint 10-13 引入的整套 compiler pipeline + 4 個 spec 的手寫檔案。

> ⚠️ **方向大轉彎**：Sprint 13 完成後，用戶反思新方向：「不需要 compiler，系統可以直接根據 json-spec 的變動而生成 schema, api, ui」。本 sprint 整個推翻 compiler 路線。

### Sprint 14 全完成（~9.5 / 9.5 SP）

#### Phase 1 — Runtime CRUD 基礎（commit `d8be716`，4.5 SP）

**Added**
- **`lib/runtime/spec-loader.ts`** — 啟動時一次載入全部 spec + cache + invalidate 機制（6 tests）
- **`lib/runtime/dynamic-handler.ts`** — 從 JsonSpec 動態組裝 list/get/create/update/delete + transition handler（5 tests）
- **`app/api/crud/[spec]/route.ts`** — Next.js dynamic segment 統一入口；id / event 走 query param（7 tests + E2E 2/2）

**Fixed**
- `extensions/order/order-spec.json` + `extensions/todo/todo-spec.json`：`options` → `validation.enum`（enum 欄位的正確位置）

#### Phase 2 — 完全移除 Compiler + 4 Spec 全切換（本次 commit，~5 SP）

**Added**
- **`lib/runtime/ui-config.ts`** — `buildListUIConfig` / `buildFormUIConfig` / `buildDetailUIConfig`（5 tests）
- **`app/admin/crud/[spec]/page.tsx`** + **`dynamic-list-client.tsx`** — 動態組裝列表頁
- **`app/admin/crud/[spec]/new/page.tsx`** + **`dynamic-form-client.tsx`** — 動態新增頁
- **`app/admin/crud/[spec]/[id]/page.tsx`** + **`dynamic-detail-client.tsx`** — 動態詳情頁（含 workflow transition）
- **`lib/ai/pipeline/pipeline-runtime.test.ts`** — pipeline 重構後測試（21 tests）
- **`tests/integration/tech-034-dynamic-ui.test.ts`** + **`tech-036b-cutover.test.ts`** — 5 + 4 tests
- **`tests/e2e/tech-034-dynamic-ui.spec.ts`** + **`tech-036b-cutover.spec.ts`** — 1 + 4 E2E

**Removed**
- **`lib/compiler/`**（5 generator + test，3656 行全刪）
- **`scripts/compile-blog-to-temp.ts`** + **`compile-order-to-temp.ts`** + **`try-compile.ts`**（3 個編譯腳本）
- **`tsconfig.test-compiler.json`**（compiler 專用 tsconfig）
- **`app/api/{blog,order,event,todo}/`**（11 個手寫 route.ts）
- **`app/admin/{blog,orders,event,todo}/`**（4 page.tsx + 11 個手寫 components）
- **`tests/integration/pipeline-integration.test.ts`**（308 行整檔刪）
- **`tests/integration/compiler-blog-compile.test.ts`**（整檔刪）

**Changed**
- **`lib/ai/pipeline/pipeline-runner.ts`** — 重構：5 stages 改為 runtime 指向；新增 `PipelineStageError`（附 stage 名）+ `shouldSkip` 條件跳過 + 失敗時 `value=null` + `error.stage=name`
- **`lib/runtime/ui-config.ts`** — workflow 雙軌取法 `spec.workflows?.[0] ?? model.workflows?.[0]`
- **`lib/runtime/dynamic-handler.ts`** — 修正殘留舊「generated by compiler」註解
- **`lib/specs/json-spec.types.ts`** — `apiBase` / `uiBase` 標為 `@deprecated`（保留向後兼容）
- **`extensions/{blog,order,event,todo}/manifest.json`** — `nav.path` 統一改為 `/admin/crud/<spec>`
- **`extensions/{blog,order}/spec.json`** — 移除 `apiBase` / `uiBase`
- **`extensions/{event,todo}/spec.json`** — 補 `requiresExtension`（Sprint 9 漏掉，disable guard 才真的生效）
- **`prisma/seed-orders.ts`** — 修正 console.log
- **5 個既有 integration 測試**（blog-extension、event-extension、blog-mixed-mode、todo-extension、three-cruds-e2e）改為驗證 runtime + 手動維護 Prisma
- **`tests/integration/tech-025-order-spec.test.ts`** — 重寫：只保留 spec.json 結構驗證
- **5 個既有 E2E/integration 測試**（admin-sidebar、extension-nav、disable-guard-sidebar、disable-guard-api、tech-025）更新路徑指向 `/admin/crud/<spec>` + `?id=&event=` query param

### Sprint 14 揭露並修正的問題

1. **event / todo spec 缺少 `requiresExtension`** — Sprint 9 false claim 揭露。E2E disable-guard-api.spec 測試時發現 disable event 後 GET 仍 200。手動驗證後補完。
2. **`setExtensionEnabled` race bug** — `current.isEnabled` 是 closure 變數沒在 while loop 內重 fetch。修為每次重 fetch。
3. **Sprint 14 設計改變導致既有測試預期失效** — disable error code 從 `ExtensionDisabled` 常數改為描述性 string；disable page guard 從 redirect 改為顯示訊息。修測試期望。

### Sprint 14 測試基線

| 項目 | Sprint 13 結束 | Sprint 14 結束 | 增減 |
|---|---|---|---|
| vitest | 809 / 61 | **719 / 60** | -90 / -1（移除 compiler 相關 + 整檔刪 2） |
| E2E | 22 | **29** | +7（tech-034 1 + tech-036b 4 + disable-guard fix 2） |
| Typecheck | ✅ 綠 | ✅ 綠 | 全綠 |
| Compiler | 5 generator | **0** | 全移除 ✅ |
| 手寫 API/admin | 19 files | **0** | 全切換 ✅ |
| 總測試 | 831 | **748** | -83（含 2 整檔刪 -308+8） |
| 真實可用性 | 「typecheck 過綠 ≠ 真能用」 | **dev server 手動驗證 4 spec 全 E2E 綠** | 第一次真實驗證 ✅ |

### Sprint 14 路線圖

| Task | 狀態 |
|---|---|
| TECH-031 spec-loader | ✅ Phase 1 |
| TECH-032 dynamic-handler | ✅ Phase 1 |
| TECH-033 catch-all route | ✅ Phase 1 |
| TECH-034 dynamic UI page | ✅ Phase 2 |
| TECH-035 移除 compiler + 重構 pipeline | ✅ Phase 2 |
| TECH-036b 4 spec 全切換 | ✅ Phase 2 |

完整 reflection 見 [`docs/reflection/sprint-14.md`](docs/reflection/sprint-14.md)。
